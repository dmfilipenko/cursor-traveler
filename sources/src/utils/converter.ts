...
export const convertMetrics = cond([
  [lte(10 ** 5), pipe(cmToM, mToKm, fixed(3), addMetric(UnitsFull.KILOMETR))],
  [lte(10 ** 2), pipe(cmToM, fixed(2), addMetric(UnitsFull.METER))],
  [T, pipe(fixed(2), addMetric(UnitsFull.CENTIMETER))]
])
